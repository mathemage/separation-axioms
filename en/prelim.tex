...
\begin{rem}[Sublocale homomorphisms]
  ...
  \begin{itemize}
    ...
    In this representation \emph{closed sublocales\/} are the~frame
...
\begin{lem} \label{lem:embed-adjoint}
  The embedding $j\colon \left\uparrow a \right. \embed L$ is the right adjoint
  to~$\check{a}$.
\end{lem}
\begin{proof}
...
\begin{proof}
  ...
  Set $\beta := (x \to c \vee x)\colon \left\uparrow a\right. \to
  \left\uparrow c\right.$.
  We have $\beta\check{a}(x) = c \vee a \vee x = c \vee x = \check{c}(x)$,
