...
\begin{itemize}
\item $\N$ resp. $\Q$ is the set of~all natural resp. rational numbers.
...
\begin{exmpl}
  ...
  0, 1\rangle$.
  This topological space is denoted by~$\I$.
\end{exmpl}
...
\item A \emph{sublocale} of a~locale $L$ is $S\subseteq L$ satisfying
  \begin{description}
...
\begin{proof}
  ...
  let us find $\gamma\colon \left\uparrow c \right. \to M$ with $\gamma
  \check{c} = h$.
...
every system $(x_i)_{i\in J}$ in~$L_1$ resp. $L_2$ and every $y$ in $L_2$ resp.
$L_1$ we have
\begin{align*}
