...
\begin{rem} \label{DSHaus->Haus}
  ...
  \[
    ...
    \right\},
  \]
  the rest of the proof can easily be finished in nearly the same way.
\end{rem}
...
\begin{proof}
  ...
  \]
  for every $a\in L$.
  From the definition of the~infimum, $\Delta(0_L) \subseteq \bigwedge
  \Delta[L]$.
\end{proof}
...
\]
which---having denoted the henceforth crucial element $\Delta(0_L)$ by
$d_L$---produces the final
...
\begin{cor}
  Strongly Hausdorff locale $\Omega(X)$ makes the space $X$ Hausdorff.
\end{cor}

\begin{proof}
  Following from \ref{IHaus->DSHaus} and \ref{DSHaus->Haus}.
\end{proof}
