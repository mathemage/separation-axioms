...
\begin{rem} \label{rem:delta-restriction}
  For the monotone $\Delta$ it holds that $\Delta(0) \le \Delta(a)$ for $a\in
  L$.
  Thus, we always have $\Delta[L] \subseteq \dL$ and may correctly define
  \[
    \overline{\Delta}
  \]
  as the restriction of~$\Delta$ to~$L \to \dL$.
\end{rem}
...
\begin{proof}
  ...
  It is, therefore, an epimorphism.

  Furthermore, $\nabla$~and~$\gamma$ are frame homomorphism (using asociativity
  for joins and~distributivity for finite meets);
  and since $\nabla$ is onto, $\alpha$ is also a~frame homomorphism.
  %(recall the proposition on page~\pageref{TBD}).

  From a~property of Galois adjoints $\nabla \Delta (0) \le 0$, we have
  \[
    \nabla \alpha \nabla(U)
    = \nabla \gamma(U)
    = \nabla (U \vee d_L)
  ...
  \]
  that is, $\nabla \alpha = id$.
  Hence, $\alpha$ is one-one, and thus, an isomorphism inverse to the~$\nabla$
  restricted~to $\dL \to L$.
