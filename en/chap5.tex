...
\begin{framed}
  \begin{df}[$T_3$]
    A~topological space~$X$ is \emph{regular\/} (or satisfies the
    \emph{$T_3$-axiom\/}) if
    \begin{center} \it
      for any $x\in X$ and any closed $A$ such that $A \not\owns x$ there are
      disjoint open $V_1, V_2$ such that $V_1\owns x$ and $V_2\supseteq A$.
    \end{center}
  \end{df}
\end{framed}
...
\begin{proof}
  ...
  consequently, completes the proof of~the~inclusion $\subseteq$.

  $\Leftarrow$:
  With $U := X\setminus A$ take $V_x$ from above and set $V_1 := V_x, \; V_2 :=
...
\begin{lem} \label{lem:rb-char}
  $\overline{V}\subseteq U \quad \equiv \quad \exists W\in \Omega(X), \quad W
  \cap V = \none \;\; \& \;\; W \cup U = X$.
\end{lem}
\begin{proof}
  $\Rightarrow$:
  ...
  yet the $W$ does not intersect the $V$.
  Thus, $z\not\in \overline{V}$.
\end{proof}
...
\end{lem}
\begin{proof}
  Using the distributivity we have
  \[
...
\begin{proof}
  (i):
  Recall that $a \oplus b = \iota_1(a) \wedge \iota_2(b)$.
  We have
  \begin{align*}
  ...
  and as we have the frame distributivity (for the third equality).
...
\begin{lem}
  For a general locale $L$ and any of its saturated $U \supseteq d_L$ we have
  \[
...
\begin{proof}
  ...
  By the previous lemma, we have $(x, y)\in U$ for every $x \rb a$ and $y \rb
  b$.
...
\begin{proof}
  ...
  a$.
  Hence, for the sake of~contradiction, let us assume $a \not\le \bigvee \{x
  \st x \rb a\}$. Then there exists $c$ from the~premise.
...
\begin{proof}
  Let $a \not\le b$.
  From Lemma~\ref{reg-char} we get an~element $c$ such that $a \vee c = 1$ and
  $c^* \not\le b$.
  What is more, it satisfies $b \vee c \ne 1$.
  If not so then by the distributivity
  \[
...
\begin{rem}
  ...
  page~\pageref{thm:what-does-sfit-do} the regularity implies all Hausdorff
...
\begin{proof}
  ...
  \[
    a \rb b 
    \; \equiv \;
    \exists c, \quad a \wedge c = 0 \text{ and } c \vee b = 1.
  \]
  ...
  $
  The other inequality is seen from~\ref{rb->leq}.
